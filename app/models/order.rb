...
class Order < ApplicationRecord
  ...
  belongs_to :collaboration, lambda {
    with_deleted.joins(:order).where(orders: { parent_type: 'Collaboration' })
  ...
  def redsys_order_id
    @redsys_order_id ||=
      if redsys_response && first
        ...
        parent.id.to_s.rjust(7, '0') + Order::PARENT_CLASSES[parent.class] + Time.now.to_i.to_s(36)[-4..]
      end
  end
