...
class UsersMailer < ApplicationMailer
  ...
  def microcredit_email(microcredit, loan, brand_config)
    ...
    attachments[pdf_name] =
      WickedPdf.new.pdf_from_string(render_to_string(template: 'microcredit/email_guide', formats: [:pdf],
                                                     encoding: 'UTF-8'))
    mail(from: @brand_config['mail_from'], to: @loan.email,
         subject: t('microcredit.email.subject', name: @brand_config['name']))
  end
  ...
  def remember_email(type, query)
    @user = case type
            ...
            when :document_vatid
              User.find_by(document_vatid: query)
            else
              User.find_by(email: query)
            end
    mail(
