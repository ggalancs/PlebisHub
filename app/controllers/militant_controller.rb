...
class MilitantController < ApplicationController
  ...
  def get_militant_info
    ...
    if params[:participa_user_id].blank?
  ...
  def handle_exemption_update(user, exemption_param)
    ...
    begin
      ...
      render json: { error: 'Failed to update user', details: e.message }, status: :unprocessable_content
    end
  end
