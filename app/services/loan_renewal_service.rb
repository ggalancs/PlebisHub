...
class LoanRenewalService
  ...
  def find_initial_loan(loan_id, current_user)
    if loan_id
      MicrocreditLoan.find_by(id: loan_id)
    elsif current_user
      MicrocreditLoan.renewables.where(document_vatid: current_user.document_vatid).first ||
        MicrocreditLoan.recently_renewed.where(document_vatid: current_user.document_vatid).first
    end
  end
  ...
  def populate_renewal_data(renewal, loan)
    renewal.loans = MicrocreditLoan.renewables.where(
    ...
    renewal.other_loans = MicrocreditLoan.renewables
                                         .where.not(microcredit_id: loan.microcredit_id)
                                         .where(document_vatid: loan.document_vatid)
                                         .to_a
                                         .uniq(&:microcredit_id)

    renewal.recently_renewed_loans = MicrocreditLoan.recently_renewed.where(
      microcredit_id: loan.microcredit_id,
      document_vatid: loan.document_vatid
    )
