...
ActiveAdmin.register_page 'CensusTool' do
  ...
  page_action :search_document_vatid, method: [:post] do
    ...
    paper_vote_user = User.confirmed.not_banned.militant.where(vote_circle_id: current_user.vote_circle_id).where(
      'lower(document_vatid) = ?', dn.downcase
    ).find_by(document_type: dt)
