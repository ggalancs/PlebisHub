...
ActiveAdmin.register Collaboration do
  ...
  sidebar 'Acciones', 'data-panel' => :collapsed, only: :index, priority: 0 do
    status = Collaboration.has_bank_file? Time.zone.today
    ...
    ul do
      ...
      li do
        this_month = Order.banks.by_date(Time.zone.today, Time.zone.today).charging.count
  ...
  collection_action :download_csv, method: :get do
    status = Collaboration.has_bank_file? Time.zone.today
    if status[1]
      send_file Collaboration.bank_filename Time.zone.today
    else
  ...
  collection_action :download_for_town, method: :get do
    ...
    Order.paid.group(:town_code, Order.unique_month('payable_at')).order(:town_code, Order.unique_month('payable_at')).pluck(
      'town_code', Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
  ...
  collection_action :download_for_autonomy, method: :get do
    ...
    Order.paid.where(town_code: nil, island_code: nil).group('autonomy_code', Order.unique_month('payable_at')).order('autonomy_code', Order.unique_month('payable_at')).pluck(
      'autonomy_code', Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
  ...
  collection_action :download_for_island, method: :get do
    ...
    Order.paid.where(town_code: nil).group('island_code', Order.unique_month('payable_at')).order('island_code', Order.unique_month('payable_at')).pluck(
      'island_code', Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
  ...
  collection_action :download_for_vote_circle_town, method: :get do
    ...
    provinces = Carmen::Country.coded('ES').subregions
    towns_data = Hash.new { |h, k| h[k] = Hash.new 0 }
    Order.paid.where("target_territory like 'Municipal%'").group(:vote_circle_town_code, Order.unique_month('payable_at')).order(:vote_circle_town_code, Order.unique_month('payable_at')).pluck(
      'vote_circle_town_code', Order.unique_month('payable_at'), 'count(id) as count_id', 'sum(amount) as sum_amount'
    ).each do |c, m, t, v|
      towns_data[c][m.to_i] = [t, v]
    end
    Order.paid.where("target_territory like 'Municipal%'").where(vote_circle_town_code: nil).where.not(town_code: nil).group(:town_code, Order.unique_month('payable_at')).order(:town_code, Order.unique_month('payable_at')).pluck(
      'town_code', Order.unique_month('payable_at'), 'count(id) as count_id', 'sum(amount) as sum_amount'
    ).each do |c, m, t, v|
  ...
  collection_action :download_for_vote_circle_autonomy, method: :get do
    ...
    Order.paid.where("target_territory like 'Autonómico%'").where(town_code: nil, vote_circle_town_code: nil, island_code: nil, vote_circle_island_code: nil).group('vote_circle_autonomy_code', Order.unique_month('payable_at')).order('vote_circle_autonomy_code', Order.unique_month('payable_at')).pluck(
      'vote_circle_autonomy_code', Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
      autonomies_data[c][m.to_i] = [t, v]
    end
    Order.paid.where(town_code: nil, vote_circle_town_code: nil, island_code: nil, vote_circle_island_code: nil, vote_circle_autonomy_code: nil).group(:autonomy_code, Order.unique_month('payable_at')).order(:autonomy_code, Order.unique_month('payable_at')).pluck(
      'autonomy_code', Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
  ...
  collection_action :download_for_vote_circle_island, method: :get do
    ...
    island_data = Hash.new { |h, k| h[k] = Hash.new 0 }
    Order.paid.where(vote_circle_town_code: nil).group('vote_circle_island_code', Order.unique_month('payable_at')).order('vote_circle_island_code', Order.unique_month('payable_at')).pluck(
      'vote_circle_island_code', Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
      island_data[c][m.to_i] = [t, v]
    end
  ...
  collection_action :download_for_circle_and_cp_town, method: :get do
    ...
    query = Order.paid.where('target_territory like ?', 'Municipal%').where.not(vote_circle_id: nil, vote_circle_town_code: nil).where('amount > 0').group(:vote_circle_town_code, :vote_circle_id, :target_territory, Order.unique_month('payable_at')).order(:vote_circle_town_code, :vote_circle_id, :target_territory, Order.unique_month('payable_at')).pluck(
      :vote_circle_town_code, :vote_circle_id, :target_territory, Order.unique_month('payable_at'), 'count(orders.id) as count_id, sum(orders.amount) as sum_amount, vote_circle_id as vc'
    )
    ...
    query = Order.paid.joins('LEFT JOIN users on orders.user_id = users.id').where('orders.target_territory like ?', 'Municipal%').where('orders.vote_circle_town_code is not null and orders.amount > 0').where(orders: { vote_circle_id: nil }).group(:vote_circle_town_code, 'users.postal_code', :target_territory, Order.unique_month('payable_at')).order(:vote_circle_town_code, 'users.postal_code', :target_territory, Order.unique_month('payable_at')).pluck(
      :vote_circle_town_code, 'users.postal_code', :target_territory, Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount'
    )
  ...
  collection_action :download_for_circle_and_cp_autonomy, method: :get do
    ...
    query = Order.paid.where('target_territory like ?', 'Autonómico%').where.not(vote_circle_id: nil, vote_circle_autonomy_code: nil).where('amount > 0').group(:vote_circle_autonomy_code, :vote_circle_id, :target_territory, Order.unique_month('payable_at')).order(:vote_circle_autonomy_code, :vote_circle_id, :target_territory, Order.unique_month('payable_at')).pluck(
      :vote_circle_autonomy_code, :vote_circle_id, :target_territory, Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount, vote_circle_id as vc'
    )
    query.each do |c, vc, tt, m, t, v|
      num_month = m.to_i
      if circle_data[c][vc][tt][num_month].zero?
    ...
    query = Order.paid.joins('LEFT JOIN users on orders.user_id = users.id').where('orders.target_territory like ?', 'Autonómico%').where('orders.vote_circle_autonomy_code is not null and orders.amount > 0').where(orders: { vote_circle_id: nil }).group('users.vote_town', 'users.postal_code', :target_territory, Order.unique_month('payable_at')).order('users.vote_town', 'users.postal_code', :target_territory, Order.unique_month('payable_at')).pluck(
      'users.vote_town', 'users.postal_code', :target_territory, Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount'
    )
    query.each do |c, cp, tt, m, t, v|
      num_month = m.to_i
      if towns_data[c][cp][tt][num_month].zero?
    ...
    # -------------------------- Add Non User data --------------------------------------------------------------------------------
    c_ids = Order.paid.joins('LEFT JOIN users on orders.user_id = users.id').where('orders.target_territory like ?',
    ...
    Collaboration.where(id: c_ids).find_each do |collaboration|
      query = Order.paid.where(parent_id: collaboration.id).group(:target_territory, Order.unique_month('payable_at')).order(:target_territory, Order.unique_month('payable_at')).pluck(
        :target_territory, Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount'
      )
  ...
  collection_action :download_for_circle_and_cp_country, method: :get do
    ...
    query = Order.paid.where('target_territory like ?', 'Estatal%').where.not(vote_circle_id: nil).where(vote_circle_autonomy_code: nil, vote_circle_island_code: nil, vote_circle_town_code: nil).where('amount > 0').group(:vote_circle_id, :target_territory, Order.unique_month('payable_at')).order(:vote_circle_id, :target_territory, Order.unique_month('payable_at')).pluck(
      :vote_circle_id, :target_territory, Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount, vote_circle_id as vc'
    )
    ...
    query = Order.paid.joins('LEFT JOIN users on orders.user_id = users.id').where('orders.target_territory like ?', 'Estatal%').where(vote_circle_autonomy_code: nil, vote_circle_island_code: nil, vote_circle_town_code: nil).where('orders.amount > 0').where(orders: { vote_circle_id: nil }).group('users.vote_town', 'users.postal_code', :target_territory, Order.unique_month('payable_at')).order('users.vote_town', 'users.postal_code', :target_territory, Order.unique_month('payable_at')).pluck(
      'users.vote_town', 'users.postal_code', :target_territory, Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount'
    )
    query.each do |c, cp, tt, m, t, v|
      next unless cp
    ...
    # -------------------------- Add Non User data --------------------------------------------------------------------------------
    non_user_base = Order.paid.joins('LEFT JOIN users on orders.user_id = users.id').where('orders.target_territory like ?', 'Estatal%').where(
    ...
    Collaboration.with_deleted.where(id: c_ids).find_each do |collaboration|
      query = non_user_base.where(parent_id: collaboration.id).group(:target_territory, Order.unique_month('payable_at')).order(:target_territory, Order.unique_month('payable_at')).pluck(
        :target_territory, Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount'
      )
      query.each do |tt, m, t, v|
        non_user = collaboration.get_non_user
  ...
  batch_action :error_batch, if: proc { params[:scope] == 'suspects' } do |ids|
    ...
    Collaboration.transaction do
      Collaboration.where(id: ids).find_each do |c|
