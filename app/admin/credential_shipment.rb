...
ActiveAdmin.register_page 'Envios de Credenciales' do
  ...
  page_action :generate_shipment, method: :get do
    ...
    us.each do |r|
      ...
      row_users_data[r.user_id] =
        [r.user_id.to_s, "#{r.first_name.capitalize} #{r.last_name.capitalize}", r.first_name.capitalize.to_s,
         r.last_name.capitalize.to_s, r.address, r.postal_code.to_s, " #{u.town_name}", u.province_name.to_s, r.phone, code]
    ...
    csv = CSV.generate(encoding: 'utf-8', col_sep: "\t") do |csv|
      header1 = ['Id', 'Nombre y Apellidos', 'Nombre', 'Apellidos', 'Dirección', 'Código Postal', 'Municipio', 'Provincia',
                 'Teléfono', 'Código Credencial']
      csv << header1

      row_users_data.each_value do |data|
        row = data
        csv << row
      end
    end
    ...
    send_data csv.encode('utf-8'),
              ...
              disposition: "attachment; filename=credentials_created_at_.#{Time.zone.today}.csv"
  end
end
