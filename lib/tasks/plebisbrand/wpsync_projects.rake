...
namespace :plebisbrand do
  ...
  task export_projects: :environment do
    last_update = Time.zone.today - 7.years
    ...
    loop do
      ImpulsaProject.exportable.where('updated_at>?', last_update).order(id: :asc).each do |project|
        begin
          ...
          project.wizard_export.each do |key, value|
            if key == 'wizard_title'
              ...
              attachment = { name: File.basename(value),
                             ...
                             bits: XMLRPC::Base64.new(File.read(path)),
          ...
          pos = if pr_category == 'HACEMOS'
                  1
                else
                  pr_category2 == 'Impulsa tu entorno' ? 4 : 5
                end
          pr_territory = project.impulsa_edition_category.name.split.slice(pos)
          post_info[:custom_fields] << { key: 'project_territory', value: pr_territory || '' }
          post_info[:custom_fields] << { key: 'project_email',
                                         value: if project.user
                                                  project.user.email
                                                else
                                                  project.wizard_values['autoridad.email_contacto'] || project.wizard_values['persona.email'] || project.wizard_values['circulo.email']
                                                end }
          post_info[:custom_fields] << { key: 'project_evaluation_result',
                                         value: project.evaluation_result || '' }
          post_info[:post_name] = post_info[:post_title].parameterize
          post_info[:terms_names][:edition_category] =
            [project.impulsa_edition.name, project.impulsa_edition_category.name]

          ret = wp.execute('plebisbrand.saveProject',
                           { project_id: project.id, project: post_info, attachment: attachment })
          p "S #{project.id} -> #{ret}"
        rescue Exception => e
          p "ERROR IN #{project.id} -> #{e}"
        end
