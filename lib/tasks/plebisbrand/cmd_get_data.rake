...
namespace :cmd_get_data do
  ...
  task phones: :environment do
    ...
    def validate_phone(phone_to_validate)
      min_phone_number = 34_600_000_000
      max_phone_number = 34_799_999_999
      correct_phone_length = 9
      phone_prefix = '34'

      phone = phone_to_validate.to_i.to_s
      lon = phone.length
      phone = phone_prefix + phone if lon == correct_phone_length
    ...
    def get_phones_to_sms(options)
      return unless options.keys.many?
      ...
      if options[:postal_code] && valid_json?(options[:postal_code])
        options[:postal_code] =
          JSON.parse(options[:postal_code])
      end

      if options.key?(:ayuda)
      ...
      if options[:cccaa] && spain
        ...
        options[:cccaa].each do |ccaa|
          midata = PlebisBrand::GeoExtra::AUTONOMIES.map do |k, v|
            next unless v[0] == ccaa
      ...
      if options[:foreign].blank? && options[:cccaa].blank? && options[:province].present?
        data = data.where('vote_town ilike ?',
                          "m_#{options[:province]}%")
      end
      if options[:foreign].blank? && options[:cccaa].blank? && options[:province].blank? && options[:town].present?
        data = data.where(vote_town: options[:town])
      end
      if options[:foreign].blank? && options[:cccaa].blank? && options[:province].blank? && options[:town].blank? && options[:postal_code].present?
        data = data.where(postal_code: options[:postal_code])
      end

      results = []
      data.each do |u|
        i += 1
        print("\r#{i}") if (i % 100).zero?
        ...
        if options[:vote_town_validated]
          result = (if (u10 && u10.vote_autonomy_code != '' && u10.vote_autonomy_code == options[:cccaa]) || (u10 && u10.vote_town != '' && u10.vote_town == options[:town])
                      result
                    end)
        end
        results.push result
      ...
      split_each = options[:split_each] || 45_000
      ...
      system(command) if i > split_each
