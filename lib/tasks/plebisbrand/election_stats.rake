...
def export(name, total, col_sep = "\t", force_quotes = false)
  elections = Election.order(id: :asc)
  FileUtils.mkdir_p('tmp/export') unless File.directory?('tmp/export')
  CSV.open("tmp/export/#{name}.csv", 'w',
    ...
    elections.each do |e|
      total[e.id].each do |town, totals|
        ...
        writer << [e.id, town, totals[:inscritos], totals[:activos], totals[:votos], e.title, u.vote_town_name,
                   u.vote_province_name, u.vote_autonomy_name]
      end
    end
  end
end
...
namespace :plebisbrand do
  ...
  task :election_stats, [:ids] => :environment do |_t, args|
    ...
    User.with_deleted.includes(:versions).find_each(batch_size: batch_size) do |u|
      if u.id > max_id
        ...
        Vote.with_deleted.where('user_id between ? and ?', u.id, max_id).find_each do |v|
      ...
      elections.each do |e|
        ...
        elsif e.has_valid_user_created_at?(user) && (e.scope.zero? || e.has_valid_location_for?(user))
          total[e.id][town][:inscritos] += 1
          total[e.id][town][:activos] += 1 if user.current_sign_in_at && user.current_sign_in_at > lsd
        end
      end

      export("election-stats-#{i}", total) if (i % 100).zero? && (i % 1000).zero?
    end
