...
RSpec.describe ImpulsaProjectStateTransition, type: :model do
  ...
  describe 'CRUD operations' do
    it 'creates impulsa_project_state_transition with valid attributes' do
      ...
      }.to change(ImpulsaProjectStateTransition, :count).by(2)
    end
    ...
    it 'deletes impulsa_project_state_transition' do
      ...
      expect {
        transition.destroy
      }.to change(ImpulsaProjectStateTransition, :count).by(-1)
    end
  end
  ...
  describe 'workflow' do
    it 'tracks state transition history' do
      ...
      expect(last_three.map(&:from)).to eq(['draft', 'submitted', 'under_review'])
      expect(last_three.map(&:to)).to eq(['submitted', 'under_review', 'approved'])
    end
