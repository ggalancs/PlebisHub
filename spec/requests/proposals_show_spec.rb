  ...
  describe 'GET /es/propuestas/:id' do
    describe 'A. RENDERING BÁSICO' do
      ...
      it 'si renderiza, muestra título de Iniciativas Ciudadanas' do
        ...
        if response.status == 200
          expect(response.body).to include('Iniciativas')
        end
      end
    end
    ...
    describe 'B. INFORMACIÓN DEL AUTOR' do
      ...
      it 'si renderiza, tiene imagen del autor' do
        get '/es/propuestas/1'
        if response.status == 200
          expect(response.body).to match(/author-default\.png|image_tag/)
        end
      end
      ...
      it 'si renderiza, muestra fecha de creación' do
        get '/es/propuestas/1'
        if response.status == 200
          expect(response.body).to match(/hace|date/)
        end
      end
    end
    ...
    describe 'C. CONTENIDO DE LA PROPUESTA' do
      ...
      it 'si renderiza, muestra descripción de la propuesta' do
        get '/es/propuestas/1'
        if response.status == 200
          has_description = response.body.include?('description') || response.body.match?(/<p>.*<\/p>/m)
      ...
      it 'si renderiza, tiene enlace a debate en Plaza PlebisBrand' do
        get '/es/propuestas/1'
        if response.status == 200
          expect(response.body).to match(/Debate.*Plaza|reddit_url/i)
        end
      end
      ...
      it 'si renderiza, muestra tiempo restante o finalización' do
        get '/es/propuestas/1'
        if response.status == 200
          expect(response.body).to match(/Termina|Cerrada/i)
        end
      end
    end
    ...
    describe 'D. SIDEBAR DE INFORMACIÓN' do
      ...
      it 'si renderiza, muestra umbrales de apoyo' do
        get '/es/propuestas/1'
        if response.status == 200
          expect(response.body).to match(/Umbrales|0,2%|2%|10%|20%/)
        end
      end
      ...
      it 'si renderiza, menciona Plaza PlebisBrand' do
        get '/es/propuestas/1'
        if response.status == 200
          expect(response.body).to include('Plaza')
        end
      end
      ...
      it 'si renderiza, muestra porcentaje de apoyos necesarios' do
        get '/es/propuestas/1'
        if response.status == 200
          expect(response.body).to match(/apoyos necesarios|10% necesario/)
        end
      end
    end
    ...
    describe 'F. BOTÓN DE APOYO' do
      it 'si renderiza propuesta no finalizada, muestra formulario de apoyo' do
        get '/es/propuestas/1'
        if response.status == 200 && !response.body.include?('finished?')
    ...
    describe 'H. ESTRUCTURA HTML' do
      ...
      it 'si renderiza, usa clase proposal' do
        get '/es/propuestas/1'
        if response.status == 200
          expect(response.body).to match(/class=".*proposal/)
        end
      end
    end
  end
