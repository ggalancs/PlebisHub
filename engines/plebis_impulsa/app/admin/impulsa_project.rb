...
ActiveAdmin.register PlebisImpulsa::ImpulsaProject, as: 'ImpulsaProject' do
  ...
  permit_params do
    ...
    if params[:review] == 'true'
      # last param could be a hash for multivalue fields
      all_fields.concat(all_fields.pop.keys) if all_fields.last.is_a?(Hash)
      ps.concat(all_fields.map { |param| "_rvw#{param[4..]}" }) if resource.reviewable? # add review fields
  ...
  index download_links: -> { can?(:admin, PlebisImpulsa::ImpulsaProject) } do
    selectable_column
    column :id
  ...
  action_item(:spam, only: :show) do
    unless impulsa_project.spam?
      link_to('Marcar como Spam', spam_admin_impulsa_edition_impulsa_project_path(impulsa_edition, impulsa_project),
              method: :post, data: { confirm: '¿Estas segura de querer marcar este proyecto como Spam?' })
    end
  end

  member_action :spam, method: :post do
    p = PlebisImpulsa::ImpulsaProject.find(params[:id])
    p.mark_as_spam
    p.save
  ...
  action_item(:review, only: :show) do
    if impulsa_project.markable_for_review?
      link_to('Marcar para revision',
              review_admin_impulsa_edition_impulsa_project_path(impulsa_edition, impulsa_project), method: :post, data: { confirm: '¿Estas segura de querer marcar este proyecto para revisión?' })
    end
  end
  ...
  member_action :download_attachment do
    project = PlebisImpulsa::ImpulsaProject.find(params[:id])
    send_file project.wizard_path(params[:gname], params[:fname])
  end

  action_item(:reset_evaluator, only: :show) do
    if impulsa_project.is_current_evaluator?(current_active_admin_user.id)
      link_to('Abandonar evaluación',
              reset_evaluator_admin_impulsa_edition_impulsa_project_path(impulsa_edition, impulsa_project), method: :post, data: { confirm: 'Si abandonas se perderán los datos introducidos por ti en el formulario de evaluación. ¿Deseas continuar?' })
    end
  end
  ...
  show do
    panel t('plebisbrand.impulsa.admin_section') do
      attributes_table_for impulsa_project do
        ...
        row :category do
          link_to(impulsa_project.impulsa_edition_category.name,
                  admin_impulsa_edition_impulsa_edition_category_path(impulsa_project.impulsa_edition,
                                                                      resource.impulsa_edition_category))
        end
        row :state
        row :user do
        ...
        row :created_at
        row :updated_at
        if impulsa_project.evaluation_result.present?
          row 'Evaluación' do
    ...
    form method: :post do |_f|
      ...
      errors = impulsa_project.wizard_all_errors.to_h { |gname, fname, error| ["#{gname}.#{fname}", error] }
      impulsa_project.wizard.map do |_sname, step|
        panel step[:title] do
          step[:groups].each do |gname, group|
            next unless impulsa_project.wizard_eval_condition(group)
              ...
              group[:fields].each do |fname, field|
                ...
                row field[:title] do
                  ...
                  when 'file'
                    if value
                      div link_to(value,
                                  download_attachment_admin_impulsa_edition_impulsa_project_path(
                                    id: impulsa_project.id, fname: fname, gname: gname
                                  ))
                    else
                      div ''
                    end
                ...
                if impulsa_project.reviewable?
                  row 'revisión',
                      class: "review #{'remark' if impulsa_project.wizard_review["#{gname}.#{fname}"].present?}" do
                    textarea(impulsa_project.wizard_review["#{gname}.#{fname}"],
                             id: "impulsa_project__rvw_#{gname}__#{fname}", name: "impulsa_project[_rvw_#{gname}__#{fname}]", rows: 2)
                  end
                end
                next unless !impulsa_project.reviewable? && impulsa_project.wizard_review["#{gname}.#{fname}"].present?
      ...
      if impulsa_project.validable? || impulsa_project.evaluator[1].present?
        ...
        panel 'Evaluación', class: 'evaluation' do
          ...
          impulsa_project.evaluation.each_value do |step|
            fieldset class: 'inputs' do
              legend do
                span step[:title]
              end
                  ...
                  group[:fields].each do |fname, field|
                    ...
                    impulsa_project.evaluators.each do |i|
                      ...
                      li class: "input input_#{i}" do
                        label class: 'label' do
                          impulsa_project.evaluator[i].full_name
                        end
          ...
          if evaluator && impulsa_project.validable?
            fieldset class: :actions do
              ol do
      ...
      if impulsa_project.can_finish_evaluation?(current_active_admin_user)
        panel 'Finalizar evaluación' do
          ...
          fieldset class: :actions do
            ol do
              li class: 'action input_action' do
                input type: :submit, name: 'evaluation_action_ok', value: 'Fase superada',
                      'data-confirm' => 'Se avisará al usuario de esta decisión. ¿Deseas continuar?'
              end
              li class: 'action input_action' do
                input type: :submit, name: 'evaluation_action_ko', value: 'Fase NO superada',
                      'data-confirm' => 'Se avisará al usuario de esta decisión. ¿Deseas continuar?'
              end
            end
          end
  ...
  form do |f|
    f.inputs t('plebisbrand.impulsa.admin_section') do
      if impulsa_project.impulsa_edition
        li do
          label 'Edition'
      ...
      if impulsa_project.impulsa_edition_category
        span do
          link_to(impulsa_project.impulsa_edition_category.name,
                  admin_impulsa_edition_impulsa_edition_category_path(resource.impulsa_edition,
                                                                      resource.impulsa_edition_category))
        end
      end
      ...
      f.inputs step[:title] do
        step[:groups].each do |gname, group|
          ...
          group[:fields].each do |fname, field|
            ...
            impulsa_project.send(field_name)
            if field[:type].in? %w[select check_boxes]
              f.input field_name, label: field[:title], as: field[:type],
                                  collection: field[:collection].to_a.map(&:reverse)
            else
              f.input field_name, label: field[:title], as: field[:type]
            end
          end
  ...
  collection_action :upload_vote_results, method: :post do
    ...
    json.each do |answer|
      ...
      answer['urls'].each do |url|
        ...
        id_proyecto = id_proyecto_aux if id_proyecto_aux.positive?
      end
      votes = answer['total_count']
      votes = ((votes * 10_000) + 90_000_000).round if votes.is_a? Float
      proyecto = PlebisImpulsa::ImpulsaProject.find_by(id: id_proyecto)
      if proyecto.nil?
        no_id_projects << id_proyecto
      else
        proyecto.votes = votes
        unless answer['winner_position'].nil?
    ...
    if no_id_projects.any?
      flash[:error] =
        "Projectos no encontrados: #{no_id_projects.join(',')}. Total: #{no_id_projects.count}"
    end
    redirect_to action: :index, '[q][id_in]': procesados.join(' ').to_s, order: 'votes_desc'
  end
  ...
  controller do
    ...
    def update_scopes
      ...
      PlebisImpulsa::ImpulsaProject.state_machine.states.keys.each do |state| # rubocop:disable Style/HashEachMethods
