...
ActiveAdmin.register PlebisImpulsa::ImpulsaEdition, as: 'ImpulsaEdition' do
  menu parent: 'PlebisHubción'
  config.filters = false
  permit_params :id, :name, :email, :description, :start_at, :new_projects_until, :review_projects_until,
                :validation_projects_until, :votings_start_at, :ends_at, :publish_results_at
  ...
  index do
    ...
    column 'Proyectos' do |impulsa_edition|
      link_to("Mostrar #{impulsa_edition.impulsa_projects.count} proyectos",
              admin_impulsa_edition_impulsa_projects_path(impulsa_edition))
    end
    actions
  end
    ...
    panel t 'activerecord.models.impulsa_edition_categories' do
      table_for resource.impulsa_edition_categories.order(:name) do
        ...
        column :category_type_name do |impulsa_edition_category|
          if impulsa_edition_category.category_type_name
            t("plebisbrand.impulsa.category_type_name.#{impulsa_edition_category.category_type_name}")
          end
        end
        column :winners
        column :prize
        ...
        column :actions do |impulsa_edition_category|
          span link_to(t('active_admin.edit'),
                       edit_admin_impulsa_edition_impulsa_edition_category_path(resource, impulsa_edition_category))
          span link_to(t('active_admin.delete'),
                       admin_impulsa_edition_impulsa_edition_category_path(resource, impulsa_edition_category), method: :delete, data: { confirm: '¿Estas segura de querer borrar esta categoría?' })
        end
      end
      div link_to(t('active_admin.has_many_new', model: t('activerecord.models.impulsa_edition_category')),
                  new_admin_impulsa_edition_impulsa_edition_category_path(resource))
    end
    ...
    panel t 'activerecord.models.impulsa_edition_topics' do
      table_for resource.impulsa_edition_topics do
        ...
        column :actions do |topic|
          ...
          span link_to(t('active_admin.delete'), admin_impulsa_edition_impulsa_edition_topic_path(resource, topic),
                       method: :delete, data: { confirm: '¿Estas segura de querer borrar esta temática?' })
        end
      end
      div link_to(t('active_admin.has_many_new', model: t('activerecord.models.impulsa_edition_topics')),
                  new_admin_impulsa_edition_impulsa_edition_topic_path(resource))
    end
  end
  ...
  action_item(:create_election, only: :show) do
    link_to('Crear votaciones', create_election_admin_impulsa_edition_path(impulsa_edition),
            data: { confirm: '¿Estas segura de querer crear las votaciones para esta edición de IMPULSA?' })
  end

  member_action :create_election do
    p = PlebisImpulsa::ImpulsaEdition.find(params[:id])
...
ActiveAdmin.register PlebisImpulsa::ImpulsaEditionTopic, as: 'ImpulsaEditionTopic' do
  menu false
