...
module PlebisImpulsa
  module ImpulsaProjectStates
    ...
    included do
      scope :exportable, -> { where state: %i[validated winner] }
      ...
      state_machine initial: :new do
        ...
        state :new, :review, :spam do
          def editable?
            !persisted? || (!resigned? && impulsa_edition.allow_edition?)
          end
        end

        state all - %i[new review spam] do
          def editable?
            false
          end
        end
      ...
      def fixable?
        persisted? && !resigned? && fixes? && impulsa_edition.allow_fixes?
      end
    end
  end
end
