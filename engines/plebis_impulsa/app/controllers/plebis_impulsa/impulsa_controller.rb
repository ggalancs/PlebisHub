...
module PlebisImpulsa
  ...
  class ImpulsaController < ApplicationController
    ...
    before_action :verify_project_ownership,
                  only: %i[download upload delete_file update update_step review delete]
    before_action :validate_step, only: %i[project_step update_step]
    ...
    def upload
      ...
      unless params[:field] =~ /\A[a-z0-9_]+\.[a-z0-9_]+\z/i
        ...
        return render json: [t('impulsa.errors.invalid_field')], status: :unprocessable_content
      end

      gname, fname = params[:field].split('.')

      # Validate file is present
      return render json: [t('impulsa.errors.no_file_provided')], status: :unprocessable_content if params[:file].blank?
    ...
    def set_variables
      ...
      @project = ImpulsaProject.new user: current_user if @project.nil? && @edition.allow_creation?
      ...
      if @project.present?
        ...
        @project.assign_attributes(project_params) if params[:impulsa_project].present?
      end
    rescue StandardError => e
    ...
    def verify_project_ownership
      return if @project && @project.user_id == current_user.id
    ...
    def validate_step
      ...
      return if valid_steps.include?(@step)
