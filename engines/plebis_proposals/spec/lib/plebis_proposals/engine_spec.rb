...
RSpec.describe PlebisProposals::Engine, type: :rails_engine do
  describe 'engine configuration' do
    ...
    it 'has the correct namespace' do
      expect(described_class.railtie_namespace).to eq(PlebisProposals)
    end
  end
  ...
  describe 'initializers' do
    ...
    it 'defines plebis_proposals.check_activation initializer' do
      initializer = initializers.find { |i| i.name == 'plebis_proposals.check_activation' }
  ...
  describe 'activation check' do
    it 'handles enabled engine' do
      ...
      result = ::EngineActivation.enabled?('plebis_proposals')
