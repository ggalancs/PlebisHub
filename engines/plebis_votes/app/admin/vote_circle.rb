...
ActiveAdmin.register PlebisVotes::VoteCircle, namespace: :admin do
  ...
  form do |f|
    f.semantic_errors
    f.inputs 'Details' do
      input :kind, as: :select, collection: PlebisVotes::VoteCircle.kinds.map{|k,v| [k.capitalize,PlebisVotes::VoteCircle.kinds.key(v)]}, selected: resource.kind, include_blank: false, label: 'tipo de círculo'
      input :original_name
  ...
  controller do
    ...
    def change_children_vote_circle(resource)
      default_id = PlebisVotes::VoteCircle.where(code: DEFAULT_VOTE_CIRCLE).pluck(:id).first
    ...
    def assign_vote_circle_territory(vc)
      ...
      exterior_circles = PlebisVotes::VoteCircle.all.where("code not like all(array[?])",known_types).where(country_code:nil, autonomy_code: nil,province_code: nil)
