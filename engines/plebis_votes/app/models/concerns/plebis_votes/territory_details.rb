  ...
  module TerritoryDetails
    ...
    def get_valid_town_code(town_code, country_code = 'ES', generate_dc = false)
      ...
      if begin
        ...
        false
      end && town_code.to_i.between?(1000, 529_999)
        town_code = town_code.to_s + calc_muni_dc(town_code).to_s if town_code.to_i < 53_000 && generate_dc
      ...
      if defined?(Carmen) && result
        ...
        result  = nil unless country.subregions[result[2, 2].to_i - 1].subregions.coded(result)
      end
    ...
    def territory_details(options)
      ...
      if options.is_a?(Hash)
        options = { town_code: options[:town_code] || nil, country_code: options[:country_code] || 'ES',
                    generate_dc: options[:generate_dc] || false, unknown: options[:unknown] || unknown, result_as: options[:result_as] || :hash }
      elsif options.is_a?(Numeric) || options.is_a?(String)
        generate_dc = (options.to_s.length == 5)
        options = { town_code: options, country_code: 'ES', generate_dc: generate_dc, unknown: unknown,
                    result_as: :hash }
      else
      ...
      result = { town_code: town_code, town_name: town_name, province_code: province_code, province_name: province_name,
                 autonomy_code: autonomy_code, autonomy_name: autonomy_name }
