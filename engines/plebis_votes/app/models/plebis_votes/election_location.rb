# frozen_string_literal: true

module PlebisVotes
  class ElectionLocation < ApplicationRecord
    belongs_to :election, class_name: 'PlebisVotes::Election'
    has_many :election_location_questions, lambda {
      order(:id)
    }, class_name: 'PlebisVotes::ElectionLocationQuestion', dependent: :destroy

    # SECURITY NOTE SEC-030: Nested attributes security
    # This accepts_nested_attributes_for is protected by:
    # 1. Strong parameters in engines/plebis_votes/app/admin/election.rb (line 201)
    #    - Whitelists only: :id, :_destroy, :title, :description, :voting_system, :layout,
    #                       :winners, :minimum, :maximum, :random_order, :totals, :options, options_headers
    # 2. :reject_if => :all_blank prevents creation of empty nested records
    # 3. :allow_destroy => true is safe as it's controlled by admin-only interface
    # 4. Access controlled by ActiveAdmin authentication (admin users only)
    accepts_nested_attributes_for :election_location_questions, reject_if: :all_blank, allow_destroy: true

    validates :title, :layout, :theme, presence: true, if: -> { has_voting_info }

    LAYOUTS = { 'simple' => 'Listado de respuestas simple',
                'accordion' => 'Listado de respuestas agrupadas por categoría',
                'pcandidates-election' => 'Listado respuestas agrupadas por categoría y pregunta',
                'simultaneous-questions' => 'Listado de preguntas con 2 respuestas',
                '2questions-conditional' => 'Pregunta con 2 respuestas, si se elige la segunda puede aparecer otra con hasta 4 respuestas' }.freeze
    ELECTION_LAYOUTS = %w[pcandidates-election 2questions-conditional].freeze

    def self.themes
      @@themes ||= Rails.application.secrets.agora['themes']
    end

    after_initialize do
      unless persisted?
        self.agora_version = 0 if agora_version.nil?
        self.new_agora_version = agora_version if new_agora_version.nil?
        self.location = '00' if location.blank?
        self.has_voting_info = false
        self.layout = LAYOUTS.keys.first
        self.theme = ElectionLocation.themes.first
      end
      self.has_voting_info = title.present?
    end

    attr_reader :has_voting_info

    def has_voting_info=(value)
      @has_voting_info = [true, 'true', '1'].include?(value)
    end

    before_save do
      clear_voting unless has_voting_info
    end

    def clear_voting
      self.title = self.layout = self.description = self.share_text = self.theme = nil
      election_location_questions.destroy_all
    end

    def territory
      spain = Carmen::Country.coded('ES')
      case election.scope
      when 0
        'Estatal'
      when 1
        autonomy = PlebisBrand::GeoExtra::AUTONOMIES.values.uniq.select { |a| a[0][2..] == location }.first
        autonomy[1]
      when 2
        province = spain.subregions[location.to_i - 1]
        province.name
      when 3
        town = spain.subregions[location[0..1].to_i - 1].subregions.coded(format('m_%s_%s_%s', location[0..1],
                                                                                 location[2..4], location[5]))
        town.name
      when 4
        island = PlebisBrand::GeoExtra::ISLANDS.values.uniq.select { |i| i[0][2..] == location }.first
        island[1]
      when 5
        'Exterior'
      when 6
        vote_circle = PlebisVotes::VoteCircle.where(id: location.to_i).first
        "#{I18n.t('vote_circle.vote_circle')} #{vote_circle.name}"
      end + " (#{location})"
    rescue StandardError
      location
    end

    def new_version_pending
      agora_version != new_agora_version
    end

    def vote_location
      if election.scope == 3
        location[0..4]
      else
        location
      end
    end

    def vote_id
      "#{election.agora_election_id}#{override.presence || vote_location}#{agora_version}".to_i
    end

    def new_vote_id
      "#{election.agora_election_id}#{override.presence || vote_location}#{new_agora_version}".to_i
    end

    def link
      "#{election.server_url}booth/#{vote_id}/vote"
    end

    def new_link
      "#{election.server_url}booth/#{new_vote_id}/vote"
    end

    def election_layout
      if ELECTION_LAYOUTS.member? layout
        layout
      else
        ''
      end
    end

    def valid_votes_count
      election.votes.with_deleted.where(agora_id: vote_id).where('deleted_at is null or deleted_at>?',
                                                                 election.ends_at).select(:user_id).distinct.count
    end

    def counter_token
      @counter_token ||= election.generate_access_token("#{created_at.to_i} #{id}")
    end

    def paper_token
      @paper_token ||= election.generate_access_token("#{created_at.to_i} #{id}")
    end
  end
end
