  ...
  describe 'logging' do
    context 'when engine is disabled' do
      it 'logs info message when engine is disabled' do
        ...
        expect(Rails.logger).to receive(:info).with('[PlebisVotes] Engine disabled, skipping routes')
        Rails.logger.info('[PlebisVotes] Engine disabled, skipping routes') unless ::EngineActivation.enabled?('plebis_votes')
      end
    end
    ...
    context 'when EngineActivation check fails' do
      it 'logs warning with error message' do
        ...
        expect(Rails.logger).to receive(:warn).with(/\[PlebisVotes\] Could not check activation status/)
        begin
          ::EngineActivation.enabled?('plebis_votes')
