...
module PlebisCollaborations
  class Collaboration < ApplicationRecord
    ...
    def payment_type_name
      PlebisCollaborations::Order::PAYMENT_TYPES.invert[self.payment_type]
      # TODO
    end

    def frequency_name
      FREQUENCIES.invert[self.frequency]
    end
    ...
    def create_order date, maybe_first=false,add_amount=true
      ...
      date = date.change(day: PlebisCollaborations::Order.payment_day) unless is_first and self.is_credit_card?
      ...
      order = PlebisCollaborations::Order.new user: self.user,
        parent:self,
    ...
    def must_have_order? date
      ...
      if self.first_order.nil?
        ...
        next_order += 1 if self.is_bank? and self.created_at.unique_month==next_order and self.created_at.day >= PlebisCollaborations::Order.payment_day
    ...
    def parse_non_user
      @non_user = if self.non_user_data
        YAML.safe_load(self.non_user_data, permitted_classes: [PlebisCollaborations::Collaboration::NonUser, Symbol], aliases: true)
      else
        nil
      end
    end
