...
module PlebisCollaborations
  class Collaboration < ApplicationRecord
    ...
    validates :non_user_document_vatid, uniqueness: { case_sensitive: false, scope: :deleted_at },
                                        allow_blank: true, unless: :skip_queries_validations
    validates :non_user_email, :non_user_document_vatid, :non_user_data, presence: true, if: proc { |c|
      c.user.nil?
    }
    ...
    def is_bank_international?
      has_iban_account? and !iban_account.start_with?('ES')
    end
    ...
    def payment_type_name
      PlebisCollaborations::Order::PAYMENT_TYPES.invert[payment_type]
      # TODO
    end
    ...
    def is_payable?
      [2,
       3].include? status and deleted_at.nil? and valid? and (!user or user.deleted_at.nil?)
    end
    ...
    def create_order(date, maybe_first = false, add_amount = true)
      ...
      if frequency == 1
        ...
        last_returned_order = orders.where('payed_at > ?',
      ...
      date = date.change(day: PlebisCollaborations::Order.payment_day) unless is_first && is_credit_card?
      ...
      order = PlebisCollaborations::Order.new user: user,
                                              parent: self,
    ...
    def processed_order!(error = nil, warn = false, is_error = false)
      ...
      oids = orders.pluck(:id).last(MAX_RETURNED_ORDERS)
      returned_orders = orders.where(id: oids).returned.count
      ...
      if returned_orders >= MAX_RETURNED_ORDERS || is_error
        if militant
          PlebisCollaborations::CollaborationsMailer.collaboration_suspended_militant(self).deliver_now
        else
          PlebisCollaborations::CollaborationsMailer.collaboration_suspended_user(self).deliver_now
        end
      elsif militant
        PlebisCollaborations::CollaborationsMailer.order_returned_militant(self).deliver_now
      else
        PlebisCollaborations::CollaborationsMailer.order_returned_user(self).deliver_now
      end
    end
    ...
    def must_have_order?(date)
      ...
      if first_order.nil?
        ...
        if is_bank? && (created_at.unique_month == next_order) && (created_at.day >= PlebisCollaborations::Order.payment_day)
          next_order += 1
        end
    ...
    def parse_non_user
      @non_user = if non_user_data
                    YAML.safe_load(non_user_data, permitted_classes: [PlebisCollaborations::Collaboration::NonUser, Symbol],
                                                  aliases: true)
                  end
    end
    ...
    def get_vote_autonomy_code
      if user
        ...
        return nil unless get_non_user.respond_to?('ine_town')

        vote_province_code = "p_#{get_non_user.ine_town.slice(2, 2)}"
    ...
    def get_vote_island_code
      if user
        ...
        PlebisBrand::GeoExtra::ISLANDS[get_non_user.ine_town][0]
      end
    end
    ...
    def get_vote_island_name
      if user
        ...
        PlebisBrand::GeoExtra::ISLANDS[get_non_user.ine_town][1]
      end
    end
    ...
    def get_vote_circle_island_code
      ...
      if user
        ...
        island_code = PlebisBrand::GeoExtra::ISLANDS[get_non_user.ine_town][0]
      end
      island_code
    end
    ...
    def self.available_payment_types(collaboration)
      PlebisCollaborations::Order::PAYMENT_TYPES.to_a.select { |_k, v| [3, collaboration.payment_type].member?(v) }
    end
