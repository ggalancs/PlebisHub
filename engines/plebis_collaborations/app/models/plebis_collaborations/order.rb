module PlebisCollaborations
  class Order < ApplicationRecord
    ...
    belongs_to :collaboration, -> { with_deleted.joins(:order).where(orders: {parent_type: 'PlebisCollaborations::Collaboration'}) }, foreign_key: 'parent_id', class_name: "PlebisCollaborations::Collaboration"
    ...
    def payment_type_name
      PAYMENT_TYPES.invert[self.payment_type]
    end
    ...
    def self.mark_bank_orders_as_paid!(date=Date.today)
      PlebisCollaborations::Collaboration.update_paid_unconfirmed_bank_collaborations(PlebisCollaborations::Order.banks.by_date(date, date).charging)
      ...
      @order_id ||=
        if self.redsys_response and self.first
          ...
          if self.persisted?
            ...
            self.parent.id.to_s.rjust(7, "0") + PARENT_CLASSES[parent.class] + Time.now.to_i.to_s(36)[-4..-1]
          end
        end
    end
