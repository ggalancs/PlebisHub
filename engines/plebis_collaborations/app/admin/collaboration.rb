...
ActiveAdmin.register PlebisCollaborations::Collaboration, namespace: :admin do
  ...
  sidebar "Acciones", 'data-panel' => :collapsed, only: :index, priority: 0 do
    status = PlebisCollaborations::Collaboration.has_bank_file? Date.today
    ...
    ul do
      ...
      li do
        this_month = PlebisCollaborations::Order.banks.by_date(Date.today, Date.today).to_be_charged.count
        prev_month = PlebisCollaborations::Order.banks.by_date(Date.today-1.month, Date.today-1.month).to_be_charged.count
      ...
      li do
        this_month = PlebisCollaborations::Order.banks.by_date(Date.today, Date.today).charging.count
        prev_month = PlebisCollaborations::Order.banks.by_date(Date.today-1.month, Date.today-1.month).charging.count
        """Marcar como pagadas:
  ...
  collection_action :download_csv, :method => :get do
    status = PlebisCollaborations::Collaboration.has_bank_file? Date.today
    if status[1]
      send_file PlebisCollaborations::Collaboration.bank_filename Date.today
    else
  ...
  collection_action :download_for_town, :method => :get do
    ...
    PlebisCollaborations::Order.paid.group(:town_code, PlebisCollaborations::Order.unique_month('payable_at')).order(:town_code, PlebisCollaborations::Order.unique_month('payable_at')).pluck('town_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount').each do|c,m,t,v|
  ...
  collection_action :download_for_autonomy, :method => :get do
    ...
    PlebisCollaborations::Order.paid.where(town_code:nil, island_code:nil).group('autonomy_code',PlebisCollaborations::Order.unique_month('payable_at')).order('autonomy_code', PlebisCollaborations::Order.unique_month('payable_at')).pluck('autonomy_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount').each do|c,m,t,v|
  ...
  collection_action :download_for_island, :method => :get do
    ...
    PlebisCollaborations::Order.paid.where(town_code:nil).group('island_code',PlebisCollaborations::Order.unique_month('payable_at')).order('island_code', PlebisCollaborations::Order.unique_month('payable_at')).pluck('island_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount').each do|c,m,t,v|
  ...
  collection_action :download_for_vote_circle_town, :method => :get do
    ...
    provinces = Carmen::Country.coded("ES").subregions
    towns_data = Hash.new {|h,k| h[k] = Hash.new 0 }
    PlebisCollaborations::Order.paid.where("target_territory like 'Municipal%'").group(:vote_circle_town_code, PlebisCollaborations::Order.unique_month('payable_at')).order(:vote_circle_town_code, PlebisCollaborations::Order.unique_month('payable_at')).pluck('vote_circle_town_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id', 'sum(amount) as sum_amount').each do|c,m,t,v|
      towns_data[c][m.to_i]=[t,v]
    end
    PlebisCollaborations::Order.paid.where("target_territory like 'Municipal%'").where(vote_circle_town_code:nil).where.not(town_code:nil).group(:town_code, PlebisCollaborations::Order.unique_month('payable_at')).order(:town_code, PlebisCollaborations::Order.unique_month('payable_at')).pluck('town_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id', 'sum(amount) as sum_amount').each do|c,m,t,v|
  ...
  collection_action :download_for_vote_circle_island, :method => :get do
    ...
    island_data = Hash.new {|h,k| h[k] = Hash.new 0 }
    PlebisCollaborations::Order.paid.where(vote_circle_town_code:nil).group('vote_circle_island_code',PlebisCollaborations::Order.unique_month('payable_at')).order('vote_circle_island_code', PlebisCollaborations::Order.unique_month('payable_at')).pluck('vote_circle_island_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount').each do|c,m,t,v|
      island_data[c][m.to_i] = [t,v]
    end
  ...
  collection_action :download_for_circle_and_cp_autonomy, :method => :get do
    ...
    towns_data = Hash.new {|h,k| h[k] = Hash.new{|h,k| h[k] = Hash.new{|h,k| h[k] = Hash.new{|h,k| h[k]= 0}}}}
    query = PlebisCollaborations::Order.paid.joins("LEFT JOIN users on orders.user_id = users.id").where("orders.target_territory like ?",'Autonómico%').where("orders.vote_circle_autonomy_code is not null and orders.amount > 0").where("orders.vote_circle_id is null").group('users.vote_town', 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order('users.vote_town', 'users.postal_code',:target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck('users.vote_town', 'users.postal_code',:target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount')
    query.each do|c,cp,tt,m,t,v|
      num_month = m.to_i
      if towns_data[c][cp][tt][num_month] == 0
        towns_data[c][cp][tt][num_month] = [t,v]
    ...
    # -------------------------- Add Non User data --------------------------------------------------------------------------------
    c_ids = PlebisCollaborations::Order.paid.joins("LEFT JOIN users on orders.user_id = users.id").where("orders.target_territory like ?",'Autonómico%').where("orders.vote_circle_autonomy_code is not null and orders.amount > 0").where("orders.vote_circle_id is null").where("users.id is null").pluck(:parent_id).uniq!
    PlebisCollaborations::Collaboration.where(id:c_ids).each do |collaboration|
      query = PlebisCollaborations::Order.paid.where(parent_id: collaboration.id).group(:target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order(:target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck(:target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount')
  ...
  collection_action :download_for_circle_and_cp_country, :method => :get do
    ...
    towns_data = Hash.new {|h,k| h[k] = Hash.new{|h,k| h[k] = Hash.new{|h,k| h[k] = Hash.new{|h,k| h[k]= 0}}}}
    query = PlebisCollaborations::Order.paid.joins("LEFT JOIN users on orders.user_id = users.id").where("orders.target_territory like ?",'Estatal%').where( vote_circle_autonomy_code: nil, vote_circle_island_code: nil, vote_circle_town_code: nil).where("orders.amount > 0").where("orders.vote_circle_id is null").group('users.vote_town', 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order('users.vote_town', 'users.postal_code',:target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck('users.vote_town', 'users.postal_code',:target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount')
    query.each do|c,cp,tt,m,t,v|
      next unless cp
      num_month = m.to_i
      if towns_data[c][cp][tt][num_month] == 0
    ...
    # -------------------------- Add Non User data --------------------------------------------------------------------------------
    non_user_base = PlebisCollaborations::Order.paid.joins("LEFT JOIN users on orders.user_id = users.id").where("orders.target_territory like ?",'Estatal%').where( vote_circle_autonomy_code: nil, vote_circle_island_code: nil, vote_circle_town_code: nil).where("orders.amount > 0 and orders.vote_circle_id is null and users.id is null")
    c_ids = non_user_base.pluck(:parent_id)
    PlebisCollaborations::Collaboration.with_deleted.where(id:c_ids).each do |collaboration|
      query = non_user_base.where(parent_id:collaboration.id).group(:target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order(:target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck(:target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount')
      query.each do|tt,m,t,v|
        non_user = collaboration.get_non_user
        c = non_user.ine_town
  ...
  batch_action :error_batch, if: proc{ params[:scope]=="suspects" } do |ids|
    ...
    PlebisCollaborations::Collaboration.transaction do
      PlebisCollaborations::Collaboration.where(id:ids).each do |c|
