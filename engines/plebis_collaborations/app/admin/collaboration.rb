...
def show_order(o, html_output = true)
  text = if o.has_errors?
           ...
           'd'
         elsif o.is_chargeable? || !o.persisted?
           '_'
         else
           '~'
         end
  text = link_to(text, admin_order_path(o)).html_safe if o.persisted? && html_output
  text
end

def show_collaboration_orders(collaboration, html_output = true)
  today = Time.zone.today.unique_month
  output = (collaboration.get_orders(Time.zone.today - 6.months, Time.zone.today + 6.months).map do |orders|
    ...
    month_orders = orders.sort_by do |o|
      o.created_at or Date.civil(2100, 1, 1)
    end.map { |o| show_order o, html_output }.join
    if html_output
...
ActiveAdmin.register PlebisCollaborations::Collaboration, namespace: :admin do
  ...
  sidebar 'Acciones', 'data-panel' => :collapsed, only: :index, priority: 0 do
    status = PlebisCollaborations::Collaboration.has_bank_file? Time.zone.today
  ...
  filter :status, as: :select, collection: PlebisCollaborations::Collaboration::STATUS.to_a
  filter :frequency, as: :select, collection: PlebisCollaborations::Collaboration::FREQUENCIES.to_a
  ...
  show do |collaboration|
    attributes_table do
      ...
      row :circulo do |collaboration|
        if collaboration.get_user && collaboration.get_user.vote_circle_id.present?
          collaboration.get_user.vote_circle.original_name
        end
      end
      row :created_at
      row :updated_at
  ...
  form do |f|
    f.inputs 'Colaboración' do
      f.input :user_id
      f.input :status, as: :select, collection: PlebisCollaborations::Collaboration::STATUS.to_a
      f.input :amount, as: :radio, collection: PlebisCollaborations::Collaboration::AMOUNTS.to_a # , input_html: {disabled: true}
      f.input :frequency, as: :radio, collection: PlebisCollaborations::Collaboration::FREQUENCIES.to_a # , input_html: {disabled: true}
      f.input :payment_type, as: :radio, collection: PlebisCollaborations::Order::PAYMENT_TYPES.to_a # , input_html: {disabled: true}
      f.input :ccc_entity
      f.input :ccc_office
      f.input :ccc_dc
      f.input :ccc_account
  ...
  collection_action :download_csv, method: :get do
    status = PlebisCollaborations::Collaboration.has_bank_file? Time.zone.today
    if status[1]
      send_file PlebisCollaborations::Collaboration.bank_filename Time.zone.today
    else
  ...
  collection_action :mark_as_charged, method: :get do
    ...
    PlebisCollaborations::Order.mark_bank_orders_as_charged! date
    redirect_to :admin_collaborations
  end

  collection_action :mark_as_paid, method: :get do
    date = Date.parse params[:date]
    PlebisCollaborations::Order.mark_bank_orders_as_paid! date
    redirect_to :admin_collaborations
  end
  ...
  collection_action :process_bank_response, method: :post do
    ...
    items.each do |item|
      ...
      order_id = item.at_xpath('OrgnlTxRef/MndtRltdInf/MndtId').text[4..].to_i
      ...
      order = PlebisCollaborations::Order.find(order_id)
      result = if order
      ...
      messages << { result: result, order: order, ret_code: code, account: "#{iban}/#{bic}",
                    fullname: fullname }
    rescue StandardError
      messages << { result: :error, info: item, message: $ERROR_INFO.message }
    end
  ...
  action_item(:charge_collaboration, only: :show) do
    if resource.is_credit_card?
      link_to 'Cobrar', charge_order_admin_collaboration_path(id: resource.id),
              data: { confirm: 'Se enviarán los datos de la orden para que esta sea cobrada. ¿Deseas continuar?' }
    else
  ...
  action_item(:restore_collaboration, only: :show) do
    if collaboration.deleted?
      link_to('Recuperar colaboración borrada', recover_admin_collaboration_path(collaboration), method: :post,
                                                                                                 data: { confirm: '¿Estas segura de querer recuperar esta colaboración?' })
    end
  end

  member_action :recover, method: :post do
    collaboration = PlebisCollaborations::Collaboration.with_deleted.find(params[:id])
  ...
  csv do
    ...
    column :collabortion_type do |collaboration|
      if collaboration.for_island_cc
        'I'
      elsif collaboration.for_town_cc
        'M'
      else
        collaboration.for_autonomy_cc ? 'A' : 'E'
      end
    end
    ...
    column :amount_current do |collaboration|
      ...
      if collaboration.is_payable? && collaboration.must_have_order?(Time.zone.today)
  ...
  collection_action :download_for_town, method: :get do
    date = Date.parse params[:date]
    months = (0..5).map do |i|
      [(date - i.months).unique_month, (date - i.months).strftime('%b').downcase]
    end.reverse.to_h
    provinces = Carmen::Country.coded('ES').subregions
    towns_data = Hash.new { |h, k| h[k] = Hash.new 0 }
    PlebisCollaborations::Order.paid.group(:town_code, PlebisCollaborations::Order.unique_month('payable_at')).order(:town_code, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'town_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
    ...
    send_csv_file(headers, months, output_data, "plebisbrand.for_town_cc.#{Time.zone.today}.csv")
  end
  ...
  collection_action :download_for_autonomy, method: :get do
    ...
    autonomies = PlebisBrand::GeoExtra::AUTONOMIES.values.to_h
    autonomies['~'] = 'Sin asignación'
    autonomies_data = Hash.new { |h, k| h[k] = Hash.new 0 }

    PlebisCollaborations::Order.paid.where(town_code: nil, island_code: nil).group('autonomy_code', PlebisCollaborations::Order.unique_month('payable_at')).order('autonomy_code', PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'autonomy_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
    ...
    send_csv_file(headers, months, output_data, "plebisbrand.for_autonomy_cc.#{Time.zone.today}.csv")
  end
  ...
  collection_action :download_for_island, method: :get do
    ...
    islands.each_value(&:uniq!)

    provinces = Carmen::Country.coded('ES').subregions

    island_data = Hash.new { |h, k| h[k] = Hash.new 0 }
    PlebisCollaborations::Order.paid.where(town_code: nil).group('island_code', PlebisCollaborations::Order.unique_month('payable_at')).order('island_code', PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'island_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
    ...
    provinces.each_with_index do |province, i|
      ...
      islands[prov_code].each do |island_code, island_name|
        ...
        months.each_key do |month|
          Rails.logger.debug { "#{island_code} #{month}" }
    ...
    send_csv_file(headers, months, output_data, "plebisbrand.for_island_cc.#{Time.zone.today}.csv")
  end
  ...
  collection_action :download_for_vote_circle_town, method: :get do
    ...
    provinces = Carmen::Country.coded('ES').subregions
    towns_data = Hash.new { |h, k| h[k] = Hash.new 0 }
    PlebisCollaborations::Order.paid.where("target_territory like 'Municipal%'").group(:vote_circle_town_code, PlebisCollaborations::Order.unique_month('payable_at')).order(:vote_circle_town_code, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'vote_circle_town_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id', 'sum(amount) as sum_amount'
    ).each do |c, m, t, v|
      towns_data[c][m.to_i] = [t, v]
    end
    PlebisCollaborations::Order.paid.where("target_territory like 'Municipal%'").where(vote_circle_town_code: nil).where.not(town_code: nil).group(:town_code, PlebisCollaborations::Order.unique_month('payable_at')).order(:town_code, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'town_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id', 'sum(amount) as sum_amount'
    ).each do |c, m, t, v|
    ...
    headers = ['Comunidad Autónoma', 'Provincia', 'Municipio']
    send_csv_file(headers, months, output_data, "circulos.for_town_cc.#{Time.zone.today}.csv")
  end
  ...
  collection_action :download_for_vote_circle_autonomy, method: :get do
    ...
    PlebisCollaborations::Order.paid.where("target_territory like 'Autonómico%'").where(town_code: nil, vote_circle_town_code: nil, island_code: nil, vote_circle_island_code: nil).group('vote_circle_autonomy_code', PlebisCollaborations::Order.unique_month('payable_at')).order('vote_circle_autonomy_code', PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'vote_circle_autonomy_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
      autonomies_data[c][m.to_i] = [t, v]
    end
    PlebisCollaborations::Order.paid.where(town_code: nil, vote_circle_town_code: nil, island_code: nil, vote_circle_island_code: nil, vote_circle_autonomy_code: nil).group(:autonomy_code, PlebisCollaborations::Order.unique_month('payable_at')).order(:autonomy_code, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'autonomy_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
    ...
    headers = ['Comunidad Autónoma']
    send_csv_file(headers, months, output_data, "circulos.for_autonomy_cc.#{Time.zone.today}.csv")
  end
  ...
  collection_action :download_for_vote_circle_island, method: :get do
    ...
    island_data = Hash.new { |h, k| h[k] = Hash.new 0 }
    PlebisCollaborations::Order.paid.where(vote_circle_town_code: nil).group('vote_circle_island_code', PlebisCollaborations::Order.unique_month('payable_at')).order('vote_circle_island_code', PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'vote_circle_island_code', PlebisCollaborations::Order.unique_month('payable_at'), 'count(id) as count_id, sum(amount) as sum_amount'
    ).each do |c, m, t, v|
      island_data[c][m.to_i] = [t, v]
    end
    ...
    headers = ['Comunidad Autónoma', 'Provincia', 'Isla']
    send_csv_file(headers, months, output_data, "circulos.for_island_cc.#{Time.zone.today}.csv")
  end
  ...
  collection_action :download_for_circle_and_cp_town, method: :get do
    ...
    query = PlebisCollaborations::Order.paid.where('target_territory like ?', 'Municipal%').where.not(vote_circle_id: nil, vote_circle_town_code: nil).where('amount > 0').group(:vote_circle_town_code, :vote_circle_id, :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order(:vote_circle_town_code, :vote_circle_id, :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      :vote_circle_town_code, :vote_circle_id, :target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id, sum(orders.amount) as sum_amount, vote_circle_id as vc'
    )
    query.each do |c, vc, tt, m, t, v|
      num_month = m.to_i
      if circle_data[c][vc][tt][num_month].zero?
    ...
    provinces.each_with_index do |province, i|
      prov_code = "p_#{(i + 1).to_s.rjust(2, '0')}"
      province.subregions.each do |town|
        circle_data[town.code].each_key do |vc|
          vote_circle = VoteCircle.find(vc)
          tts = circle_data[town.code][vc].keys
          tts = [''] if tts.none?
          tts.each do |tt|
            row = [PlebisBrand::GeoExtra::AUTONOMIES[prov_code][1], province.name, town.name, vote_circle.original_name,
                   '', tt]
            sum_row = 0
            months.each_key do |month|
    ...
    query = PlebisCollaborations::Order.paid.joins('LEFT JOIN users on orders.user_id = users.id').where('orders.target_territory like ?', 'Municipal%').where('orders.vote_circle_town_code is not null and orders.amount > 0').where(orders: { vote_circle_id: nil }).group(:vote_circle_town_code, 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order(:vote_circle_town_code, 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      :vote_circle_town_code, 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount'
    )
    query.each do |c, cp, tt, m, t, v|
      num_month = m.to_i
      if towns_data[c][cp][tt][num_month].zero?
    ...
    provinces.each_with_index do |province, i|
      prov_code = "p_#{(i + 1).to_s.rjust(2, '0')}"
      province.subregions.each do |town|
        towns_data[town.code].each_key do |cp|
          ...
          tts.each do |tt|
            ...
            output_data << row if sum_row.positive?
          end
        end
      end
    end
    ...
    send_csv_file(headers, months, output_data, "plebisbrand.user_for_cp_cc.#{Time.zone.today}.csv")
  end
  ...
  collection_action :download_for_circle_and_cp_autonomy, method: :get do
    ...
    query = PlebisCollaborations::Order.paid.where('target_territory like ?', 'Autonómico%').where.not(vote_circle_id: nil, vote_circle_autonomy_code: nil).where('amount > 0').group(:vote_circle_autonomy_code, :vote_circle_id, :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order(:vote_circle_autonomy_code, :vote_circle_id, :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      :vote_circle_autonomy_code, :vote_circle_id, :target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount, vote_circle_id as vc'
    )
    query.each do |c, vc, tt, m, t, v|
      num_month = m.to_i
      if circle_data[c][vc][tt][num_month].zero?
    ...
    autonomies.sort.each do |autonomy_code, autonomy|
      circle_data[autonomy_code].each_key do |vc|
        vote_circle = VoteCircle.find(vc)
    ...
    query = PlebisCollaborations::Order.paid.joins('LEFT JOIN users on orders.user_id = users.id').where('orders.target_territory like ?', 'Autonómico%').where('orders.vote_circle_autonomy_code is not null and orders.amount > 0').where(orders: { vote_circle_id: nil }).group('users.vote_town', 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order('users.vote_town', 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'users.vote_town', 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount'
    )
    query.each do |c, cp, tt, m, t, v|
      num_month = m.to_i
      if towns_data[c][cp][tt][num_month].zero?
  ...
  collection_action :download_for_circle_and_cp_country, method: :get do
    ...
    provinces = Carmen::Country.coded('ES').subregions
    PlebisBrand::GeoExtra::AUTONOMIES.values.to_h
    Carmen::Country.all.to_h do |c|
      [c.code, c.name]
    end
    output_data = []
    ...
    query = PlebisCollaborations::Order.paid.where('target_territory like ?', 'Estatal%').where.not(vote_circle_id: nil).where(vote_circle_autonomy_code: nil, vote_circle_island_code: nil, vote_circle_town_code: nil).where('amount > 0').group(:vote_circle_id, :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order(:vote_circle_id, :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      :vote_circle_id, :target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount, vote_circle_id as vc'
    )
    query.each do |vc, tt, m, t, v|
      num_month = m.to_i
      if circle_data[vc][tt][num_month].zero?
    ...
    query = PlebisCollaborations::Order.paid.joins('LEFT JOIN users on orders.user_id = users.id').where('orders.target_territory like ?', 'Estatal%').where(vote_circle_autonomy_code: nil, vote_circle_island_code: nil, vote_circle_town_code: nil).where('orders.amount > 0').where(orders: { vote_circle_id: nil }).group('users.vote_town', 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order('users.vote_town', 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
      'users.vote_town', 'users.postal_code', :target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount'
    )
    query.each do |c, cp, tt, m, t, v|
      next unless cp
    ...
    # -------------------------- Add Non User data --------------------------------------------------------------------------------
    non_user_base = PlebisCollaborations::Order.paid.joins('LEFT JOIN users on orders.user_id = users.id').where('orders.target_territory like ?', 'Estatal%').where(
      vote_circle_autonomy_code: nil, vote_circle_island_code: nil, vote_circle_town_code: nil
    ).where('orders.amount > 0 and orders.vote_circle_id is null and users.id is null')
    c_ids = non_user_base.pluck(:parent_id)
    PlebisCollaborations::Collaboration.with_deleted.where(id: c_ids).find_each do |collaboration|
      query = non_user_base.where(parent_id: collaboration.id).group(:target_territory, PlebisCollaborations::Order.unique_month('payable_at')).order(:target_territory, PlebisCollaborations::Order.unique_month('payable_at')).pluck(
        :target_territory, PlebisCollaborations::Order.unique_month('payable_at'), 'count(orders.id) as count_id', 'sum(orders.amount) as sum_amount'
      )
      query.each do |tt, m, t, v|
        non_user = collaboration.get_non_user
        c = non_user.ine_town
        c = non_user.town_name if c.blank?
        c = 'desconocido' if c.blank?
        cp = non_user.postal_code
        cp = 'desconocido' if cp.blank?
    ...
    towns_exterior.each do |town_code|
      towns_data[town_code].each_key do |cp|
        tts = towns_data[town_code][cp].keys
        tts = [''] if tts.none?
        tts.each do |tt|
          cp_txt = cp
          cp_txt = 'desconocido' if cp.blank?
    ...
    headers = ['Comunidad Autónoma', 'Provincia', 'Municipio', 'Círculo', 'Código Postal', 'Territorio de Asignación']
    send_csv_file(headers, months, output_data, "plebisbrand.user_for_country_cp.#{Time.zone.today}.csv")
  end

  batch_action :error_batch, if: proc { params[:scope] == 'suspects' } do |ids|
    ok = true
    PlebisCollaborations::Collaboration.transaction do
      PlebisCollaborations::Collaboration.where(id: ids).find_each do |c|
    ...
    unless ok
      redirect_to(collection_path,
                  warning: 'Ha ocurrido un error y las colaboraciones no han sido marcadas como erróneas.')
    end
  end
end
