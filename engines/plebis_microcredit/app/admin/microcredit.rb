...
ActiveAdmin.register PlebisMicrocredit::Microcredit, as: 'Microcredit' do
  ...
  index do
    ...
    column :limits do |m|
      (["<strong>Total&nbsp;fase:&nbsp;#{number_to_euro(m.phase_limit_amount * 100,
                                                        0)}</strong>"] + m.limits.map do |amount, limit|
                                                                           "#{number_to_euro amount * 100,
                                                                                             0}:&nbsp;#{limit}"
                                                                         end).join('<br/>').html_safe
    end
    column :totals, text_align: 'right' do |m|
      (m.phase_status.group_by(&:first).map do |amount, info|
        "#{number_to_euro amount * 100, 0}:&nbsp;#{info.map do |x|
          "#{x[4]}#{if x[3]
                      '&#9785;'
                    else
                      (x[1] ? '&check;' : '&cross;')
                    end}#{x[2] ? '&oplus;' : '&ominus;'}"
        end.sort { |a, b| a.gsub(/\d/, '') <=> b.gsub(/\d/, '') }.join '&nbsp;'}"
    ...
    column :percentages do |m|
      (["<strong>Confianza:&nbsp;#{(m.remaining_percent * 100).round(2)}%</strong>"] + m.limits.map do |amount, _limit|
    ...
    column :progress do |m|
      ["<strong>Objetivo: #{number_to_euro m.total_goal * 100, 0}",
       "#{m.campaign_created_count}:&nbsp;#{number_to_euro(m.campaign_created_amount * 100,
                                                           0)}&nbsp;(#{number_to_percentage(
                                                             100.0 * m.campaign_created_amount / m.total_goal, precision: 2
                                                           )})</strong>",
       "#{m.campaign_confirmed_count}&check;:&nbsp;#{number_to_euro(m.campaign_confirmed_amount * 100,
                                                                    0)}&nbsp;(#{number_to_percentage(
                                                                      100.0 * m.campaign_confirmed_amount / m.campaign_created_amount, precision: 2
                                                                    )})",
       "#{m.campaign_unconfirmed_count}&cross;:&nbsp;#{number_to_euro(m.campaign_unconfirmed_amount * 100,
                                                                      0)}&nbsp;(#{number_to_percentage(
                                                                        100.0 * m.campaign_unconfirmed_amount / m.campaign_created_amount, precision: 2
                                                                      )})",
       "#{m.campaign_counted_count}&oplus;:&nbsp;#{number_to_euro(m.campaign_counted_amount * 100,
                                                                  0)}&nbsp;(#{number_to_percentage(
                                                                    100.0 * m.campaign_counted_amount / m.campaign_created_amount, precision: 2
                                                                  )})",
       "#{m.campaign_not_counted_count}&ominus;:&nbsp;#{number_to_euro(m.campaign_not_counted_amount * 100,
                                                                       0)}&nbsp;(#{number_to_percentage(
                                                                         100.0 * m.campaign_not_counted_amount / m.campaign_created_amount, precision: 2
                                                                       )})",
       "#{m.campaign_discarded_count}&#9785;:&nbsp;#{number_to_euro(m.campaign_discarded_amount * 100,
                                                                    0)}&nbsp;(#{number_to_percentage(100.0 * m.campaign_discarded_amount / m.campaign_created_amount,
                                                                                                     precision: 2)})"].join('<br/>').html_safe
    end
    actions
  end
  ...
  form do |f|
    f.inputs do
      ...
      if can? :admin, PlebisMicrocredit::Microcredit
        f.input :title
        f.input :priority
        f.input :remarked
        f.input :starts_at
  ...
  show do
    attributes_table do
      ...
      row :limits do
        (["<strong>Total&nbsp;fase:&nbsp;#{number_to_euro(microcredit.phase_limit_amount * 100,
                                                          0)}</strong>"] + microcredit.limits.map do |amount, limit|
                                                                             "#{number_to_euro amount * 100,
                                                                                               0}:&nbsp;#{limit}"
                                                                           end).join('<br/>').html_safe
      end
      row :totals do
        (microcredit.phase_status.group_by(&:first).map do |amount, info|
          "#{number_to_euro amount * 100, 0}:&nbsp;#{info.map do |x|
            "#{x[4]}#{x[1] ? '&check;' : '&cross;'}#{x[2] ? '&oplus;' : '&ominus;'}"
          end.sort do |a, b|
            a.gsub(/\d/,
                   '') <=> b.gsub(/\d/, '')
          end.join '&nbsp;'}"
      ...
      row :percentages do
        (["<strong>Confianza:&nbsp;#{(microcredit.remaining_percent * 100).round(2)}%</strong>"] + microcredit.limits.map do |amount, _limit|
      ...
      row :progress do
        ["<strong>Objetivo: #{number_to_euro microcredit.total_goal * 100, 0}",
         "#{microcredit.campaign_created_count}:&nbsp;#{number_to_euro(microcredit.campaign_created_amount * 100,
                                                                       0)}&nbsp;(#{number_to_percentage(100.0 * microcredit.campaign_created_amount / microcredit.total_goal,
                                                                                                        precision: 2)})</strong>",
         "#{microcredit.campaign_confirmed_count}&check;:&nbsp;#{number_to_euro(
           microcredit.campaign_confirmed_amount * 100, 0
         )}&nbsp;(#{number_to_percentage(
           100.0 * microcredit.campaign_confirmed_amount / microcredit.campaign_created_amount, precision: 2
         )})",
         "#{microcredit.campaign_unconfirmed_count}&cross;:&nbsp;#{number_to_euro(
           microcredit.campaign_unconfirmed_amount * 100, 0
         )}&nbsp;(#{number_to_percentage(
           100.0 * microcredit.campaign_unconfirmed_amount / microcredit.campaign_created_amount, precision: 2
         )})",
         "#{microcredit.campaign_counted_count}&oplus;:&nbsp;#{number_to_euro(microcredit.campaign_counted_amount * 100,
                                                                              0)}&nbsp;(#{number_to_percentage(100.0 * microcredit.campaign_counted_amount / microcredit.campaign_created_amount,
                                                                                                               precision: 2)})",
         "#{microcredit.campaign_not_counted_count}&ominus;:&nbsp;#{number_to_euro(
           microcredit.campaign_not_counted_amount * 100, 0
         )}&nbsp;(#{number_to_percentage(
           100.0 * microcredit.campaign_not_counted_amount / microcredit.campaign_created_amount, precision: 2
         )})",
         "#{microcredit.campaign_discarded_count}&#9785;:&nbsp;#{number_to_euro(
           microcredit.campaign_discarded_amount * 100, 0
         )}&nbsp;(#{number_to_percentage(
           100.0 * microcredit.campaign_discarded_amount / microcredit.campaign_created_amount, precision: 2
         )})"].join('<br/>').html_safe
      end
      row :mailing do
    ...
    panel "Estadísticas de la campaña #{resource.title}" do
      ids = resource.id
      total = PlebisMicrocredit::Microcredit.total_current_amount(ids)
      stats = PlebisMicrocredit::MicrocreditLoan.get_loans_stats ids
      ul do
    ...
    panel 'Lugares donde se aporta' do
      if microcredit.microcredit_options.any?
        paginated_collection(microcredit.microcredit_options.page(params[:page]).per(15), download_links: false) do
          table_for collection.order(:name) do
            ...
            column :actions do |op|
              ...
              span link_to 'Borrar', admin_microcredit_microcredit_option_path(op.microcredit, op), method: :delete,
                                                                                                    data: { confirm: '¿Estas segura de borrar esta opción?' }
            end
          end
        end
      end
  ...
  action_item(:change_phase, only: :show) do
    if resource.phase_remaining.sum(&:last) <= 0
      link_to('Cambiar de fase', change_phase_admin_microcredit_path(resource), method: :post,
                                                                                data: { confirm: '¿Estas segura de que deseas cambiar de fase en esta campaña?' })
    end
  end

  member_action :change_phase, method: :post do
    PlebisMicrocredit::Microcredit.find(params[:id]).change_phase!
  ...
  controller do
    def update
      if can? :admin, PlebisMicrocredit::Microcredit
        ...
        resource.limits = params[:microcredit].map do |k, v|
          "#{k[13..]} #{v.to_i} " if k.start_with?('single_limit_')
        end.join
  ...
  permit_params do
    if can? :admin, PlebisMicrocredit::Microcredit
      %i[title starts_at ends_at limits subgoals account_number total_goal bank_counted_amount
         contact_phone agreement_link budget_link renewal_terms mailing priority remarked]
    else
      [:contact_phone]
    end
  end
  ...
  member_action :process_bank_history, method: :post do
    ...
    norma43[:movements].each do |movement|
      ...
      mconcept = movement[:concept][38..]
      id, mname = mconcept.split(/[ -]/, 2) if mconcept

      if mconcept && mname && mname.downcase == resource.title.downcase && id.to_i.positive?
        sure = true
        temp = resource.loans.where(id: id.to_i)
      end

      if sure && temp.length == 1 && temp.first.amount == movement[:amount] && (I18n.transliterate("#{temp.first.last_name} #{temp.first.first_name}".downcase[0..37].strip) == muser || I18n.transliterate("#{temp.first.first_name} #{temp.first.last_name}".downcase[0..37].strip) == muser)
