...
RSpec.describe PlebisVerification::Engine, type: :rails_engine do
  describe 'engine configuration' do
    ...
    it 'has the correct namespace' do
      expect(described_class.railtie_namespace).to eq(PlebisVerification)
    end
  end
  ...
  describe 'initializers' do
    ...
    it 'defines plebis_verification.load_abilities initializer' do
      initializer = initializers.find { |i| i.name == 'plebis_verification.load_abilities' }
      expect(initializer).not_to be_nil
    end

    it 'defines plebis_verification.check_activation initializer' do
      initializer = initializers.find { |i| i.name == 'plebis_verification.check_activation' }
      expect(initializer).not_to be_nil
    end
  ...
  describe 'ability loading' do
    it 'loads abilities when both Ability and engine Ability are defined' do
      ...
      stub_const('PlebisVerification::Ability', engine_ability)

      Ability.register_abilities(PlebisVerification::Ability) if defined?(Ability) && defined?(PlebisVerification::Ability)

      expect(Ability.registered_abilities).to include(PlebisVerification::Ability)
    end

    it 'handles missing Ability gracefully' do
      hide_const('Ability')
      stub_const('PlebisVerification::Ability', Class.new)
    ...
    it 'handles missing engine Ability gracefully' do
      ...
      stub_const('Ability', ability_class)
      hide_const('PlebisVerification::Ability')
  ...
  describe 'activation check' do
    it 'handles enabled engine' do
      ...
      result = ::EngineActivation.enabled?('plebis_verification')
  ...
  describe 'logging' do
    context 'when engine is disabled' do
      it 'logs info message when engine is disabled' do
        ...
        expect(Rails.logger).to receive(:info).with('[PlebisVerification] Engine disabled, skipping routes')
        Rails.logger.info('[PlebisVerification] Engine disabled, skipping routes') unless ::EngineActivation.enabled?('plebis_verification')
      end
    end
    ...
    context 'when EngineActivation check fails' do
      it 'logs warning with error message' do
        ...
        expect(Rails.logger).to receive(:warn).with(/\[PlebisVerification\] Could not check activation status/)
        begin
          ::EngineActivation.enabled?('plebis_verification')
