...
module PlebisVerification
  class UserVerification < ApplicationRecord
    ...
    def rotate
      @rotate ||= ActiveSupport::HashWithIndifferentAccess.new
    end
    ...
    def validate_image_content_types
      ...
      return unless back_vatid.attached? && !back_vatid.content_type.start_with?('image/')

      errors.add(:back_vatid, 'debe ser una imagen')
    end

    def validate_image_sizes
      errors.add(:front_vatid, 'debe ser menor de 6MB') if front_vatid.attached? && front_vatid.byte_size > 6.megabytes
      return unless back_vatid.attached? && back_vatid.byte_size > 6.megabytes
    ...
    after_validation do
      errors.each do |attr|
        errors.delete(attr) if attr.to_s.starts_with?('front_vatid_') || attr.to_s.starts_with?('back_vatid_')
      end
    end
    ...
    def self.for(user, params = {})
      current = where(user: user, status: [0, 2, 3]).first
      if current
    ...
    def active?
      ...
      if current_verification && current_hash
        ...
        current_hash = current_hash.gsub(/[{}:]/, '').split(', ').map do |h|
          h1, h2 = h.split('=>')
          { h1 => h2 }
        end.reduce(:merge)
        current_hash = current_hash.transform_keys(&:to_sym)
        # end convert hash in string to hash
        DateTime.now.utc <= (current_hash[:locked_at].gsub('"', '').gsub(/[|]/,
                                                                         ':').to_datetime + Rails.application.secrets.user_verifications['time_to_expire_session'].minutes)
      else
        false
      end
    end
