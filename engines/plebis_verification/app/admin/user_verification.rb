...
ActiveAdmin.register PlebisVerification::UserVerification, as: 'UserVerification' do
  ...
  permit_params do
    params = %i[user_id processed_at publisher_id front_vatid back_vatid wants_card]
  ...
  collection_action :get_first_free, method: :get do
    ...
    if verification
      $redis.hset(:processing, verification.id,
                  { author_id: current_user.id, locked_at: DateTime.now.utc.strftime('%d/%m/%Y %H|%M') })
      redirect_to(edit_admin_user_verification_path(verification.id),
                  data: { confirm: 'Parece ser que ya tienes abierto el registro en otra pestaña, ¿realmente deseas editarlo en esta?' })
    else
      loop do
        verification = PlebisVerification::UserVerification.pending.where.not(id: ids).order(priority: :desc,
                                                                                             created_at: :asc).first
        if verification
          user_exist = User.exists?(id: verification.user_id)
          verification.update(status: 5) unless user_exist
        end
        break if !verification || user_exist
      end
      if verification
        $redis.hset(:processing, verification.id,
                    { author_id: current_user.id, locked_at: DateTime.now.utc.strftime('%d/%m/%Y %H|%M') })
        ...
        redirect_to(admin_user_verifications_path,
                    flash: { warning: t('plebisbrand.user_verification.no_pending_verifications') })
      end
    end
  end
  ...
  index do |_verification|
    ...
    if (params[:scope] == 'pendientes') || params[:scope].nil?
  ...
  show do
    columns do
      ...
      column class: 'column attachments' do
        %i[front back].each do |attachment|
          next unless user_verification.send("#{attachment}_vatid_file_name")

          div class: 'attachment' do
            a class: 'preview', target: '_blank',
              href: view_image_admin_user_verification_path(user_verification, attachment: attachment, size: :original) do
              image_tag view_image_admin_user_verification_path(user_verification, attachment: attachment,
                                                                                   size: :thumb)
            end
            div class: 'rotate' do
              span 'ROTAR'
              [0, 90, 180, 270].reverse.each do |degrees|
                a class: "degrees-#{degrees}",
                  href: rotate_admin_user_verification_path(user_verification, attachment: attachment, degrees: degrees), 'data-method' => :patch do
                  fa_icon 'id-card-o'
                end
              end
            end
    ...
    columns do
      column do
        ...
        panel 'verificar' do
          f.inputs class: 'remove-padding-top' do
            f.input :status, label: 'Estado', as: :radio, collection: if current_user.is_admin?
                                                                        {
      ...
      column class: 'column attachments' do
        ...
        if user_verification.active?
          ...
          $redis = $redis || Redis::Namespace.new('plebisbrand_queue_validator', redis: Redis.new)
          $redis.hset(:processing, user_verification.id,
                      { author_id: current_user.id, locked_at: DateTime.now.utc.strftime('%d/%m/%Y %H|%M') })
        end
        ...
        %i[front back].each do |attachment|
          div class: 'attachment' do
            ...
            unless PlebisVerification::UserVerification.statuses[resource.status] == PlebisVerification::UserVerification.statuses[:accepted]
              f.input :"#{attachment}_vatid", as: :file,
                                              required: false
            end
            div class: 'rotate' do
              span 'ROTAR'
  ...
  controller do
    def capture_redish_hash(_id)
      nil unless current_user.is_admin?
    end

    def remove_redis_hash(id)
      $redis = $redis || Redis::Namespace.new('plebisbrand_queue_validator', redis: Redis.new)
      $redis.hget(:processing, id)
    ...
    def clean_redis_hash
      ...
      ids = $redis.hkeys :processing
      ids.each do |i|
        verification = PlebisVerification::UserVerification.find_by(id: i)
    ...
    def update
      ...
      elsif (current_user.verifier? || current_user.is_admin?) && !verification.active?
        redirect_to(admin_user_verifications_path,
                    flash: { error: 'Has perdido el derecho de Verificar este usuario. Pulsa en Procesar para verificar uno nuevo.' })
      end
    end
  end
end
